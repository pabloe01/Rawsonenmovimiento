...
if (!class_exists('TC_header_main')) :
    ...
    class TC_header_main {
        ...
        function tc_favicon_display() {
            ...
            echo apply_filters('tc_favicon_display', sprintf('<link rel="shortcut icon" href="%1$s" type="%2$s">', $_fav_src, $type
                    )
            );
        }
        ...
        function tc_title_view($logo_classes) {
            ...
            printf('<%1$s><a class="site-title" href="%2$s" title="%3$s">%4$s</a></%1$s>', apply_filters('tc_site_title_tag', 'h1'), apply_filters('tc_logo_link_url', esc_url(home_url('/'))), apply_filters('tc_site_title_link_title', sprintf('%1$s | %2$s', __(esc_attr(get_bloginfo('name'))), __(esc_attr(get_bloginfo('description'))))), __(esc_attr(get_bloginfo('name')))
            );
            ...
            function tc_logo_img_view($_args) {
                ...
                extract($_args);
                $_html = sprintf('<img src="%1$s" alt="%2$s" %3$s %4$s %5$s %6$s class="%7$s %8$s"/>', $logo_src, __('Back Home', 'customizr'), $logo_width ? sprintf('width="%1$s"', $logo_width) : '', $logo_height ? sprintf('height="%1$s"', $logo_height) : '', ( 1 == $logo_resize) ? sprintf('style="max-width:%1$spx;max-height:%2$spx"', apply_filters('tc_logo_max_width', 250), apply_filters('tc_logo_max_height', 100)
                                ) : '', implode(' ', apply_filters('tc_logo_other_attributes', ( 0 == tc__f('__get_option', 'tc_retina_support') ) ? array('data-no-retina') : array() )), $logo_type, $logo_attachment_id ? sprintf('attachment-%1$s', $logo_attachment_id) : ''
                );
            ...
            printf('<a class="site-logo" href="%1$s" title="%2$s">%3$s</a>', apply_filters('tc_logo_link_url', esc_url(home_url('/'))), apply_filters('tc_logo_link_title', sprintf('%1$s | %2$s', __(esc_attr(get_bloginfo('name'))), __(esc_attr(get_bloginfo('description'))))), implode('', $logos_img)
            );
        ...
        function tc_social_in_header($resp = null) {
            ...
            $html = sprintf('<div class="social-block %1$s" %3$s>%2$s</div>', $social_header_block_class, tc__f('__get_socials'), $_nothing_to_render ? 'style="display:none"' : ''
            );
...
//end of fn
        ...
        function tc_write_header_inline_css($_css) {
            ...
            if (0 != esc_attr(tc__f('__get_option', 'tc_sticky_shrink_title_logo')) || TC_utils::$instance->tc_is_customizing()) {
                ...
                $_css = sprintf("%s\n%s", $_css, ".sticky-enabled .tc-shrink-on .site-logo img {
						{$_logo_shrink}
					}\n
            ...
            if (100 != esc_attr(tc__f('__get_option', 'tc_sticky_z_index'))) {
                ...
                $_css = sprintf("%s\n%s", $_css, ".tc-no-sticky-header .tc-header, .tc-sticky-header .tc-header {
